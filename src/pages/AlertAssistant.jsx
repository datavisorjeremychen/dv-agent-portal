...
function ChatWithAgents() {
  ...
  // ADD: result per agent (null until done)
        ...
        ag.sub.forEach((_, i) => {
          const t = setInterval(() => {
            setAgents((A) => {
              ...
              // when all sub-agents done, record grouped conclusion once
              ...
              if (allDone && agent.status === "running") {
                ...
                // Optional: still echo to chat so it's captured in the stream
                setMessages((m) => [
                  ...m,
                  botMsg(
    ...
    <>
      ...
      <div className="border-t bg-white px-3 py-2 flex items-center justify-between text-sm">
        <div className="text-slate-600">
          {agents.every((a) => a.status === "done") ? "All agents finished." : "Agents still running…"}
          {agents.some((a) => a.agree === true) ? " • You agreed with at least one conclusion." : ""}
        </div>
        <button
          className="btn btn-primary inline-flex items-center gap-1 disabled:opacity-50"
          disabled={!agents.every((a) => a.status === "done")}
          onClick={accept}
        >
